...
prs_avg=$(echo "$data_overall" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | add / length')
prs_min=$(echo "$data_overall" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | min')
prs_max=$(echo "$data_overall" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | max')
...
prs_avg=$(echo "$data_overall" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | add / length')
prs_min=$(echo "$data_overall" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | min')
prs_max=$(echo "$data_overall" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | max')
...
prs_avg=$(echo "$data_successful" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | add / length')
prs_min=$(echo "$data_successful" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | min')
prs_max=$(echo "$data_successful" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | max')
...
prs_avg=$(echo "$data_successful" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | add / length')
prs_min=$(echo "$data_successful" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | min')
prs_max=$(echo "$data_successful" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | max')
...
if [ ! "$prs_failed" = "0" ]; then
    ...
    prs_avg=$(echo "$data_failed" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | add / length')
    prs_min=$(echo "$data_failed" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | min')
    prs_max=$(echo "$data_failed" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.creationTimestamp | fromdate))] | max')
    ...
    prs_avg=$(echo "$data_failed" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | add / length')
    prs_min=$(echo "$data_failed" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | min')
    prs_max=$(echo "$data_failed" | jq -s  '[.[] | (( (.completionTime // (.finished_at | .[0:19] + "Z")) | fromdate) - (.startTime | fromdate))] | max')
...
pr_completionTime_first=$(echo $data | jq  '[.[] | (.completionTime // (.finished_at | .[0:19] + "Z") ) ] | sort | first')
pr_completionTime_last=$(echo $data | jq  '[.[] | (.completionTime // (.finished_at | .[0:19] + "Z") ) ] | sort | last')
