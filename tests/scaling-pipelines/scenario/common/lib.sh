...
function measure_signed_start() {
    ...
    ../../tools/measure-signed.py --server "$( oc whoami --show-server )" --namespace "benchmark" --token "$( oc whoami -t )" --insecure --save "./measure-signed.csv" --expect-signatures "$TEST_TOTAL" --status-data-file "benchmark-tekton.json" &
    measure_signed_pid=$!
